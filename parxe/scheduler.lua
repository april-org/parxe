...
function sched_methods:enqueue(func, ...)
  ...
  local tmpname = "%s/PX_%s_%06d_%s"%{tmp,self.HASH,task_id,os.date("%Y%m%d%H%M%S")}
  local task    = { id=task_id, func=func, args=args, wd=config.wd(), hash=self.HASH }
